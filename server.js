...
function detectPlayer(title, candidatePlayers) {
  ...
  for (const player of candidatePlayers) {
    ...
    for (const token of tokens) {
      ...
      if (parts.length >= 2) {
        // Multi-word: require both words present
...
function detectSetName(title) {
  ...
  // Check for draft indicators (college)
  if (titleLower.includes('draft')) {
    return null; // Will be flagged as college
  }
...
function passesHardGates(liveFp, compFp) {
  ...
  if (liveFp.isGraded !== compFp.isGraded) {
    return { pass: false, reason: 'graded mismatch (raw vs graded)' };
  }
...
function scoreComp(liveFp, compFp) {
  ...
  // SSP mismatch (one has very low serial, other doesn't)
...
function dedupeComps(comps) {
  ...
  for (const comp of comps) {
    ...
    // Check for exact duplicate normalized titles
    let isDupe = false;
    for (const [existingTitle, existingComp] of seen.entries()) {
...
function computeReferencePrice(comps) {
  if (comps.length < 5) {
    return { price: null, confidence: 'insufficient', reason: `only ${comps.length} comps (need 5+)` };
  }
  
  const prices = comps.map(c => c.price).sort((a, b) => a - b);
  
  // Take lowest 10 (or all if fewer)
  const lowestK = prices.slice(0, Math.min(10, prices.length));
  
  // Trimmed median: remove top/bottom 10% if enough data
  let trimmed = lowestK;
  if (lowestK.length >= 8) {
...
async function searchCompCandidates(liveFp, phase = 'strict') {
  ...
  if (phase === 'strict') {
    ...
    if (liveFp.parallelNormalized) {
      keywords += ` ${liveFp.parallelNormalized.toLowerCase().replace(/_/g, ' ')}`;
    }
    if (liveFp.serialDenom) keywords += ` /${liveFp.serialDenom}`;
  }
...
async function findCompsForListing(listing) {
  ...
  for (const cand of candidates) {
    ...
    if (score >= 70) {
      ...
      rejections['score<70'] = (rejections['score<70'] || 0) + 1;
    }
  }
  
  // Phase 2: Expansion if needed
  if (acceptedComps.length < 5) {
    console.log(`  ⚠ Only ${acceptedComps.length} comps, expanding search...`);
  ...
  console.log(`  After deduplication: ${deduped.length} comps`);
...
function evaluateDeal(listing, comps) {
  ...
  if (!refPrice) {
    console.log(`  ❌ ${confidence}: ${compsUsed || 0} comps`);
    return null;
  }
  ...
  if (ratio <= 0.85 && (confidence === 'high' || confidence === 'moderate')) {
...
app.post('/api/scan', async (req, res) => {
  ...
  console.log('\n========== CARDSNIPE SCAN ==========');
  console.log('Engine: Fingerprint + Scoring + 2-Phase Retrieval');
  console.log('Threshold: ≤85% of trimmed floor median\n');
...
/*
TEST CASE A: "2023 Prizm Bijan Robinson Red /49 RC"
Expected fingerprint:
- playerNormalized: "Bijan Robinson"
- year: "2023"
- setName: "PRIZM"
- serialDenom: "49"
- parallelNormalized: "RED_WAVE"
- rookieFlag: true
- isGraded: false

Expected comp acceptance:
✅ "2023 Panini Prizm Bijan Robinson Red Wave Rookie /49"
✅ "Bijan Robinson 2023 Prizm RC Red Wave /49"
❌ "2023 Prizm Bijan Robinson Silver RC" (parallel mismatch)
❌ "2023 Prizm Bijan Robinson Red Wave /99" (serial mismatch)
❌ "2024 Prizm Bijan Robinson Red /49" (year mismatch)

---

TEST CASE B: "2020 Prizm Stephen Curry Silver"
Expected fingerprint:
- playerNormalized: "Stephen Curry"
- year: "2020"
- setName: "PRIZM"
- parallelNormalized: "SILVER"
- isGraded: false

Expected comp acceptance:
✅ "2020 Panini Prizm Stephen Curry Silver Prizm"
✅ "Stephen Curry 2020 Prizm Silver"
❌ "2020 Prizm Stephen Curry Base" (parallel mismatch)
❌ "2020 Prizm Stephen Curry PSA 10 Silver" (graded mismatch)
❌ "2019 Prizm Stephen Curry Silver" (year mismatch)

---

TEST CASE C: "2023 Select CJ Stroud Zebra (raw)"
Expected fingerprint:
- playerNormalized: "CJ Stroud"
- year: "2023"
- setName: "SELECT"
- parallelNormalized: "ZEBRA"
- isGraded: false
- rookieFlag: likely true

Expected comp acceptance:
✅ "2023 Panini Select CJ Stroud Zebra Prizm RC"
✅ "CJ Stroud 2023 Select Zebra Rookie"
❌ "2023 Select CJ Stroud Zebra PSA 9" (graded mismatch)
❌ "2023 Select CJ Stroud Contenders Draft Zebra" (college)
❌ "2023 Select CJ Stroud Silver" (parallel mismatch)
❌ "2024 Select CJ Stroud Zebra" (year mismatch)
