  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (fingerprint.isCollege) {
        console.log('    ✗ Rejected: college card');
        continue;
      }
      
      if (fingerprint.isJunk) {
        console.log('    ✗ Rejected: junk/lot/break');
        continue;
      }
      
      if (fingerprint.noYear) {
        console.log('    ✗ Rejected: no year in title');
        continue;
      }
      
      if (!fingerprint.setName) {
        console.log('    ✗ Rejected: not Prizm/Select/Optic/NT');
        continue;
      }
      
      if (fingerprint.isGraded) {
        console.log('    ✗ Rejected: graded card');
        continue;
      }
...
function evaluateDeal(listing, soldPrices) {
  if (soldPrices.length < 2) {
    console.log('  Skipped (need 2+ comps, have ' + soldPrices.length + ')');
    return null;
  }
...
app.post('/api/scan', async function(req, res) {
  ...
  console.log('COMPS: Minimum 2 strict matches required');
  console.log('THRESHOLD: ≤75% of MEDIAN sold price\n');
...
}); + price + ' outside $40-$150 range');
        continue;
      }
      
      console.log('    ✓ ACCEPTED for comp search');
      console.log('    Fingerprint: ' + fingerprint.year + ' ' + fingerprint.setName + (fingerprint.parallel ? ' ' + fingerprint.parallel : '') + (fingerprint.cardNumber ? ' #' + fingerprint.cardNumber : ''));
