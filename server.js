...
const PARALLEL_KEYWORDS = ['silver prizm', 'silver', 'holo', 'disco', 'scope', 'mojo', 'cracked ice', 'zebra', 'tiger', 'pink ice', 'gold shimmer', 'orange ice', 'blue wave', 'red wave', 'green wave', 'purple wave', 'shimmer', 'camo', 'tie dye', 'hyper', 'neon'];
...
function normalizePlayerName(name) {
  return name.toLowerCase().replace(/[^a-z\s]/g, '').split(' ').filter(function(w) { return w.length > 1; });
}

function playerInTitle(playerName, title) {
  const playerTokens = normalizePlayerName(playerName);
  const titleLower = title.toLowerCase();
  
  let matchCount = 0;
  for (let i = 0; i < playerTokens.length; i++) {
    if (titleLower.indexOf(playerTokens[i]) !== -1) {
      matchCount++;
    }
  }
  
  return matchCount >= 2 || (playerTokens.length === 1 && matchCount === 1);
}
...
function matchFingerprints(live, comp, livePlayerName) {
  if (!playerInTitle(livePlayerName, comp.title)) {
    return { match: false, reason: 'player name mismatch' };
  }
  ...
  if (live.cardNumber) {
    if (!comp.cardNumber || live.cardNumber !== comp.cardNumber) {
      return { match: false, reason: 'card number required match (live #' + live.cardNumber + ', comp #' + (comp.cardNumber || 'missing') + ')' };
    }
  }
  
  if (live.serial) {
    if (!comp.serial || live.serial !== comp.serial) {
      return { match: false, reason: 'serial required match (live /' + live.serial + ', comp /' + (comp.serial || 'missing') + ')' };
    }
  }
  
  if (live.parallel) {
    if (!comp.parallel || live.parallel !== comp.parallel) {
      return { match: false, reason: 'parallel required match (live "' + live.parallel + '", comp "' + (comp.parallel || 'missing') + '")' };
    }
  }
...
function calculateMedian(prices) {
  ...
  const lowestN = sorted.slice(0, Math.min(8, sorted.length));
  
  const mid = Math.floor(lowestN.length / 2);
  if (lowestN.length % 2 === 0) {
    return (lowestN[mid - 1] + lowestN[mid]) / 2;
  }
  return lowestN[mid];
}
...
async function searchEbayListings(playerName) {
  try {
    ...
    console.log('  Valid cards (with anchor + player match): ' + validCards.length);
    return validCards;
...
async function getCompCandidates(playerName, listingCard) {
  try {
    ...
    keywords += ' -college -ncaa -draft -lot -break -psa -bgs -sgc -cgc -graded -slab';
...
function evaluateDeal(listing, comps) {
  ...
  if (comps.length < 3) {
    console.log('  ⚠ INSUFFICIENT (need 3+ comps, have ' + comps.length + ')');
    return null;
  }
  
  const prices = comps.map(function(c) { return c.price; });
  const floorPrice = calculateMedian(prices);
  const percentOfFloor = listing.price / floorPrice;

  if (percentOfFloor <= 0.75) {
    const percentBelow = ((1 - percentOfFloor) * 100).toFixed(1);
    console.log('  🎯 DEAL! $' + listing.price + ' vs floor $' + floorPrice.toFixed(2) + ' (' + percentBelow + '% below)');
    ...
    return {
      ...
      referenceListings: comps.map(function(c) { return c.title; }),
      timestamp: new Date().toISOString()
    };
  } else {
    console.log('  Not a deal: $' + listing.price + ' vs floor $' + floorPrice.toFixed(2) + ' (' + (percentOfFloor * 100).toFixed(0) + '% of floor)');
  }
...
app.post('/api/scan', async function(req, res) {
  ...
  console.log('\n========== CARDSNIPE SCAN ==========');
  console.log('MODE: BIN Floor Comparison (Active Listings)');
  console.log('SETS: Prizm, Select, Optic, NT (PRO ONLY)');
  console.log('FILTERS: Raw, Anchor Required, Player Match');
  console.log('THRESHOLD: ≤75% of lowest comparable BINs\n');
  
  const deals = [];
  const errors = [];

  try {
    console.log('Scanning ' + selectedPlayers.length + ' players...');
...
app.listen(PORT, function() {
  ...
  console.log('Using BIN FLOOR comparison (best available without sold data)');
