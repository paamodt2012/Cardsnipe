  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (price < 40 || price > 150) {
        if (debugCount <= 10) console.log('    ✗ Filtered: price 
...
function evaluateDeal(listing, comps) {
  ...
  if (comps.length < 3) {
    console.log('  ⚠ INSUFFICIENT CONFIDENCE (need 3+ comps, have ' + comps.length + ')');
    return null;
  }
...
app.post('/api/scan', async function(req, res) {
  ...
  console.log('MINIMUM COMPS: 3 strict matches');
